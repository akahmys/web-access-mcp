...
**Current Objective:** v0.1 is tagged and pushed. Search backend is Bing-only (`web_search`). Phase 6 (SSRF protection, PDF support, `web_fetch` caching, `batch_fetch`, page-interaction primitives, robots.txt/UA/proxy/retry hardening) is complete -- all six AWUs (6.1-6.6) landed. No open items on the roadmap right now; next work should come from a fresh review or a new user request.
